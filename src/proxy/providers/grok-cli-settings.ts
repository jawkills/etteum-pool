...
// Keep defaults in sync with GROK_CLI_REFRESH_LEAD_SEC / prior maxAccountRetries=8.
// Do NOT import from grok-cli.ts — that file imports this module (cycle).
const DEFAULT_REFRESH_LEAD_SEC = Number(process.env.GROK_CLI_REFRESH_LEAD_SEC) || 45 * 60;
const DEFAULT_MAX_ACCOUNT_RETRIES = 8;
...
export const DEFAULT_GROK_CLI_RUNTIME: GrokCliRuntimeSettings = {
  refreshLeadSec: DEFAULT_REFRESH_LEAD_SEC,
  maxAccountRetries: DEFAULT_MAX_ACCOUNT_RETRIES,
};
...
export async function getGrokCliRuntimeSettings(): Promise<GrokCliRuntimeSettings> {
  ...
  try {
    ...
    // Lazy-apply to provider singleton (dynamic import avoids cycle with grok-cli.ts).
    void import("./grok-cli").then((m) => m.applyGrokCliRuntimeSettings(value)).catch(() => {});
    return value;
  } catch (err) {
...
/** Sync snapshot — defaults until first async load. */
export function getCachedGrokCliRuntimeSettings(): GrokCliRuntimeSettings {
  return cache?.value ?? DEFAULT_GROK_CLI_RUNTIME;
}
