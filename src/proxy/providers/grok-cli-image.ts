...
export function stripGrokCliDataUrlPrefix(value: string): string {
  const s = value.trim();
 ...
 */
export function normalizeGrokCliImageRef(input: unknown): string | null {
  ...
  if (typeof input === "string") {
    ...
    return `data:image/png;base64,${stripGrokCliDataUrlPrefix(s)}`;
  }
  if (typeof input === "object") {
    const obj = input as Record<string, unknown>;
    const nested =
      obj.url ?? obj.image_url ?? obj.b64_json ?? obj.base64 ?? obj.data ?? obj.image;
    if (nested != null && nested !== input) return normalizeGrokCliImageRef(nested);
  }
  return null;
}

/** Collect image refs from OpenAI-ish image API body fields. Cap default 3. */
export function collectGrokCliImageRefs(
  ...
  const push = (v: unknown) => {
    ...
    if (Array.isArray(v)) {
      for (const item of v) {
        ...
        const n = normalizeGrokCliImageRef(item);
        if (n) out.push(n);
      }
      return;
    }
    const n = normalizeGrokCliImageRef(v);
    if (n) out.push(n);
  };
  push(body.images);
  push(body.image);
 ...
 */
export function extractGrokCliImageGenerationResults(payload: unknown): string[] {
  ...
  for (const item of output) {
    ...
    if (typeof raw === "string" && raw.trim()) {
      images.push(stripGrokCliDataUrlPrefix(raw));
      continue;
    }
    if (raw && typeof raw === "object") {
      ...
      if (typeof nested === "string" && nested.trim()) {
        images.push(stripGrokCliDataUrlPrefix(nested));
      }
    }
  }
  return images;
...
export type GrokCliUsageNormalized = {
...
export function normalizeGrokCliUsage(usage: unknown): GrokCliUsageNormalized {
...
export function emptyGrokCliUsage(): GrokCliUsageNormalized {
  return {
    prompt_tokens: 0,
    completion_tokens: 0,
    total_tokens: 0,
...
export function addGrokCliUsage(
  a: GrokCliUsageNormalized,
  b: GrokCliUsageNormalized
...
export type GrokCliImageResult = {
  success: boolean;
  imagesB64?: string[];
  usage?: GrokCliUsageNormalized;
  error?: string;
...
export type GrokCliImageRequestOpts = {
  prompt: string;
