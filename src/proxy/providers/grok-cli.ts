...
/**
 * Normalize Claude Code / Anthropic-shaped messages for OpenAI chat.
...
export function normalizeGrokCliMessagesForOpenAI(
  ...
  for (const msg of messages) {
    ...
    // Canonical OpenAI assistant with top-level tool_calls
    if (
      msg.role === "assistant" &&
    ...
    if (!Array.isArray(msg.content)) {
      // null / undefined assistant with no tool_calls → skip empty assistant
    ...
    // Anthropic tool_result blocks inside user → separate role:tool messages
    ...
    // Anthropic tool_use inside assistant → tool_calls
    ...
    // Multimodal user content stays as array; otherwise collapse to string.
    ...
    // Plain text — never emit empty assistant-only (tool-less) turns
...
export class GrokCliProvider extends BaseProvider {
  ...
  /**
   * Claude Code / Anthropic clients send content as block arrays
   * (`[{type:"text",text:"…"}]`). cli-chat-proxy OpenAI chat rejects that with
   * `400 invalid-argument Empty content block`. Flatten to OpenAI shape before wire.
   */
  private prepareChatRequest(request: ChatCompletionRequest): ChatCompletionRequest {
    const stripped = this.stripUnsupportedTools(request);
    return {
      ...stripped,
      messages: normalizeGrokCliMessagesForOpenAI(stripped.messages) as ChatCompletionRequest["messages"],
    };
  }
  ...
  private async upstreamChat(
    ...
    const req = this.prepareChatRequest(request);
